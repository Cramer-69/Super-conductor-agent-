...
## ⚠️ Not yet safe for real user data: no per-user authentication

`user_id` is a plain field in the request body — anyone who can reach
`/api/chat` or `/api/checkin` can supply any `user_id` they like and read
or write *that* user's memory. There is no token/session validation
anywhere in this service. Combined with Supabase's service-role key
(which bypasses the RLS policies in `db/schema.sql`), this means **the API
trusts whatever `user_id` it's handed**.

Set `API_SHARED_SECRET` (checked via the `X-Api-Secret` header on
`/api/chat`, `/api/checkin`, and `/api/checkin/status`) before deploying
anywhere reachable by untrusted clients — this closes off "arbitrary
internet traffic can hit the API at all" (the bigger risk once deployed
with `allow_origins=["*"]`), but **it is not per-user authentication**:
anyone with the shared secret can still supply any `user_id`. Do not treat
this as sufficient for real user data — derive `user_id` from a verified
token (the blueprint's own later phases call for Supabase Auth with
OAuth/JWT for exactly this reason) before that. Treat real per-user auth
as a blocker for any real deployment, not a nice-to-have.
